...
export default function OptionalHeader({ id, createdAt }: ITeamUnit) {
  ...
  return <div className={styles.root}>
    ...
    <div>
      ...
      <IconDelete className={styles.svg}
        onClick={async () => {
          // ninja code ;)
          await _delete(id) ? navigate(`/team`) : null;
        }} />
    </div>
  </div>
}
...
async function _delete(id: string) {
  ...
  return fetchWrapper(() => fetch(`${serviceHost('mcontent')}/api/mcontent/team/${id}`, {
    method: 'DELETE',
    headers: {
