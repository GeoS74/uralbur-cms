...
export default function OptionalHeader({ id, createdAt }: IMainSlide) {
  ...
  return <div className={styles.root}>
    ...
    <div>
      ...
      <IconEdit className={styles.svg}
        onClick={() => navigate(`/mainpage/slider/edit/${id}`)}
      />

      <IconDelete className={styles.svg}
        onClick={async () => {
          // ninja code ;)
          await _deleteSlide(id) ? navigate(`/mainpage/slider`) : null;
        }} />
    </div>
  </div>
}
...
async function _deleteSlide(id: string) {
  ...
  return fetchWrapper(() => fetch(`${serviceHost('mcontent')}/api/mcontent/slider/${id}`, {
    method: 'DELETE',
    headers: {
